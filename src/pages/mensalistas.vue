...
async function loadData() {
  loading.value = true
  try {
    const res = await axios.get(mensalistasUrl, { params: { authorization: passkey.value } })
    data.value = res.data
...
<template>
  ...
  <div v-else-if="!authenticated">
    <div class="flex flex-col md:flex-row gap-8 justify-center mb-20">
      <div>
        <div class="card bg-base-200 shadow-lg">
          <div class="card-body">
            <div class="card-actions justify-center mt-4">
              <input v-model="passkey" type="password" class="input input-bordered w-full max-w-xs" placeholder="Senha" />
              <button @click="loadData" class="btn btn-primary">Acessar</button>
            </div>
          </div>
        </div>
      </div>
